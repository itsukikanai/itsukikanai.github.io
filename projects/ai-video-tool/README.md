...
1. 解凍し、`bin` フォルダへのパスをシステム環境変数 Path に追加。
1. ターミナルで `ffmpeg -version` が通ればOKです。
...
1. 以下のいずれかの方法でノートブックを開きます。
   - **GitHubから**: このリポジトリのURLを指定して `ai_video_tool.ipynb` を開く。
   - **アップロード**: ローカルにある `ai_video_tool.ipynb` をアップロードする。
1. ノートブックの上から順にセルを実行します。
   - 途中でAPIキーの入力を求められます。
1. 最後に表示される `public URL` (例: `https://xxxx.gradio.live`) にアクセスします。
